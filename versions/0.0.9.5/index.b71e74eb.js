...
const getContext2D = ()=>{
    ...
    if (!el) throw new Error("Could not find canvas element");
    const ctx = el.getContext("2d");
    if (!ctx) throw new Error("Could not get 2d context");
    return ctx;
};
const draw = (ctx, x, y, size, color)=>{
    ctx.fillStyle = color;
    ctx.fillRect(x, y, size, size);
};
const createParticle = (ctx, particle)=>{
    return {
        ...particle,
        vx: 0,
        vy: 0
...
const randomArea = (area)=>{
    return Math.random() * area;
};
const createGroup = (ctx, n, area, color, structure)=>{
    const group = [];
    for(let i = 0; i < n; i++)group.push(createParticle(ctx, {
        id: Date.now() + i,
        x: randomArea(area),
        y: randomArea(area),
        color,
...
const move = (a, p2)=>{
    ...
    for(let j = 0; j < p2.length; j++){
        const b = p2[j];
        if (a.id === b.id || !b.alive || a.color === b.color || b.nutrient <= 0.75) continue;
        ...
        if (distance > a.size) a.energy += 0.05;
        else if (distance < a.size) a.energy -= 0.02;
        if (a.energy < 0) a.energy = 0.01;
        else if (a.energy > 1) a.energy = 0.99;
        const grav = -a.energy;
    ...
    if (a.x <= 0 || a.x >= CANVAS_SIZE - a.size) a.vx *= -1;
    if (a.y <= 0 || a.y >= CANVAS_SIZE - a.size) a.vy *= -1;
};
const reproduce = (a, p2)=>{
    const newParticles = [];
    if (!a.alive || a.children >= 2 || a.nutrient < 0.75 || p2.length >= 500) return newParticles;
    let foundCandidates = false;
    for(let j = 0; j < p2.length; j++){
        const b = p2[j];
        if (a.id === b.id || a.color !== b.color || !b.alive || b.nutrient < 0.75 || b.reproduce < 0.3) continue;
        const dx = a.x - b.x;
        const dy = a.y - b.y;
    ...
    if (a.reproduce >= 1) a.reproduce = 1;
    return newParticles;
};
const eats = (a, p2)=>{
    if (!a.alive || a.nutrient >= 0.9) return;
    let hasEaten = false;
    for(let j = 0; j < p2.length; j++){
        const b = p2[j];
        if (a.id === b.id || a.color === b.color || !b.alive || b.nutrient <= 0.25) continue;
        ...
        if (d <= a.size) {
            ...
            a.vx *= 1.01;
            b.vx *= 0.95;
            if (a.size <= START_PARTICLE_SIZE) a.size *= 1.01;
        }
    }
    if (!hasEaten) {
        a.nutrient -= 0.0001;
...
const age = (a)=>{
    ...
    if (a.age >= 1) a.alive = false;
    if (a.age >= 0.9) a.color = "gray";
};
const update = async (ctx, rules)=>{
    ...
    if (countElement) countElement.innerText = `${allParticles.length} particles`;
...
const run = async ()=>{
    ...
    const redStructure = [
        {
            x: START_PARTICLE_SIZE + START_PARTICLE_SIZE / 2,
            y: 0,
            color: "white",
            size: START_PARTICLE_SIZE * 0.5
        }, 
    ...
    await update(ctx, async (particles)=>{
        if (!paused) {
            for(let i = 0; i < particles.length; i++){
                const p = particles[i];
                if (p.size < START_PARTICLE_SIZE) p.size = START_PARTICLE_SIZE;
                eats(p, particles);
                particles.push(...reproduce(p, particles));
                move(p, particles);
                age(p);
                if (p.nutrient <= 0) p.alive = false;
                if (p.x <= 0) p.x = 0;
                else if (p.x >= CANVAS_SIZE) p.x = CANVAS_SIZE;
                if (p.y <= 0) p.y = 0;
                else if (p.y >= CANVAS_SIZE) p.y = CANVAS_SIZE;
            }
...
document.getElementById("pause")?.addEventListener("click", ()=>{
    ...
    if (el) el.textContent = paused ? "pause" : "resume";
    paused = !paused;
...
document.getElementById("add")?.addEventListener("click", ()=>{
    ...
    if (el) {
        ...
        if (span) span.textContent = addRandomParticles ? "no random particles" : "random particles";
...
document.getElementById("speed")?.addEventListener("change", (e)=>{
    ...
    if (value) speed = parseInt(value, 10);
});
run().then(()=>console.log("done"));

},{}]},["iJYvl","h7u1C"], "h7u1C", "parcelRequire1da3")
