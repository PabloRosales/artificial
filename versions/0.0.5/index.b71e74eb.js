...
const run = async ()=>{
    ...
    update(ctx, allParticles, (particles)=>{
        const newRed = particles.filter((p)=>p.color === "red");
        const newWhite = particles.filter((p)=>p.color === "white");
        const newGreen = particles.filter((p)=>p.color === "green");
        eats(red, white);
        eats(white, green);
        eats(green, red);
